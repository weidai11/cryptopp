...
static void SHA1_Transform_CXX(word32 *state, const word32 *data)
{
    word32 W[16];
...
{
    const pfnSHATransform s_pfn = InitializeSHA1Transform();
    s_pfn(state, data);
}
...
{
    const pfnSHAHashBlocks s_pfn = InitializeSHA256HashBlocks();
...
{
    const pfnSHATransform s_pfn = InitializeSHA256Transform();
    s_pfn(state, data);
}
