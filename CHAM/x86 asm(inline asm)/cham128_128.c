...
void cham128_128_setkey(void *key, void *rk)
{
	__asm
	{
		mov esi, key	// key
...
void cham128_128_encrypt(void *key, void *in)
{
	__asm
	{
		pushad
...
void cham128_128_decrypt(void *key, void *in)
{
	__asm
	{
		pushad
	...
	loop_v3 :
		ror ebp, cl	// ROR32(x[0], 1)
		mov eax, ebx
		xchg cl, ch
		rol eax, cl		// i==even일때 cl=1, odd일때 cl=8 // ROL32(x[1], 8)
		xor eax, [edi + 4 * 7]	// ROL32(x[1], 8) ^ rk[i]
		std			// DF = 1, DF가 1이면 scasd 명령 시 edi 뒤로 이동
		scasd		// 레지스터와 메모리 값 비교(edi dword만큼 이동 = rk[i-1])
		sub ebp, eax	// ROR32(x[0] ^ 8) - ((ROL32(x[1], 1) ^ rk[i % (2 * KW)]) & 0xFFFFFFFF)
		pop eax
		xor ebp, eax // (ROR32(x[0], 8) - ((ROL32(x[1], 1) ^ rk[i % (2 * KW)]) & 0xFFFFFFFF)) ^ i
		sub al, 1
		cmp al, -1
		jnz loop_v1	//	ZF=0 이면 loop_v1
